...
function ResultadosContent() {
  ...
  const filteredData = data?.filter(row => {
    ...
    if (filters.dateRange?.from && filters.dateRange?.to && row.day) {
      try {
        if (!row.day) {
          console.warn('[Filtro] Linha sem data:', row);
          return false;
        }
		
		const rowDate = parseISO(row.day);
        if (isNaN(rowDate.getTime())) {
          console.warn('[Filtro] Data inválida no parseISO:', row.day);
          return false;
        }
        ...
        if (!inRange) {
          console.log(`[Filtro] Fora do intervalo: ${row.day}`);
          return false;
        }
		
	  } catch (err) {
        console.error('[Filtro] Erro inesperado ao filtrar data:', row.day, err);
        return false;
      }
  ...
  console.table(filteredData.map(row => row.day));
